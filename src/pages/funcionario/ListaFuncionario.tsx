...
const ListaFuncionarios: React.FC = () => {
  ...
  const filteredFuncionarios = funcionarios.filter(funcionario => 
    ...
    funcionario.surName.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const handleViewDetails = (funcionario: Funcionario) => {
    const details = (
      <div>
        <p><strong>Nome:</strong> {funcionario.name} {funcionario.surName}</p>
        ...
        <p><strong>Cargo:</strong> {funcionario.role === 'ROLE_ADMIN' ? 'Admin' : 'User'}</p>
      </div>
    );
    ...
    <div className="funcionario-list">
      ...
      <table>
        <thead>
          <tr>
            <th>Nome</th>
            <th>Username</th> {/* Adicionando o cabeçalho para o username */}
            <th>RG</th>
            <th>Telefone</th>
            <th>Email</th>
            <th>Cargo</th>
        ...
        <tbody>
          {filteredFuncionarios.map((funcionario) => (
            <tr key={funcionario.id}>
              ...
              <td>{funcionario.username}</td> {/* Adicionando o username na linha */}
              <td>{funcionario.rg}</td>
              <td>{funcionario.phone}</td>
              <td>{funcionario.email}</td>
              <td>{funcionario.role === 'ROLE_ADMIN' ? 'Administrador' : 'Usuário'}</td>
              <td>
                <button className='funcionario-detail' onClick={() => handleViewDetails(funcionario)}>Ver detalhes</button>
              </td>
            </tr>
          ))}
        </tbody>
